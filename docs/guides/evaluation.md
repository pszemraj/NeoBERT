...
NeoBERT evaluation covers GLUE, MTEB, and pseudo-perplexity scoring for masked language models.
...
- Diagnostic train metrics include every microbatch since the previous evaluation. Uninterrupted checkpoint saves preserve that window, while a resumed process starts a new window because pending predictions are not checkpoint state.
...
- Output defaults to the MTEB result cache rooted at `outputs/<run>/mteb/<ckpt>/<max_length>/`; task files live at `results/NeoBERT__<model>/<contract-hash>/<task>.json`. The contract hash covers the resolved checkpoint, canonical pooling strategy, requested context length, and custom-tokenizer trust policy, so a shared `--output_folder` remains safe across checkpoints and scoring settings. `mteb_overwrite_results: true` forces recomputation for an otherwise identical contract.
- Local checkpoint evaluation requires `config.yaml` and `tokenizer/` inside the resolved step. Model architecture and token-to-ID identity come from those artifacts; launch configuration controls only evaluation settings such as task selection, pooling, batch size, requested context length, and the explicit tokenizer-code trust opt-in.
- Checkpoint-local custom tokenizer code remains disabled by default. Set launch `tokenizer.trust_remote_code: true` only after inspecting the saved tokenizer code; a local `tokenizer/` directory can still execute Python through Transformers dynamic modules.
...
Select exactly one model source: use `--hub_model <model-id>` for a Hub masked LM, or `--checkpoint_path <run-or-checkpoint-path>` for a NeoBERT checkpoint. Local evaluation requires checkpoint-local `config.yaml` and `tokenizer/`, validates vocabulary and pad-token identity, and does not accept a separate launch config that could silently disagree. Non-RoPE models are constructed with their trained positional-table size and reject evaluation beyond it; shorter evaluation contexts do not resize learned embeddings. Hub sources accept `--revision`, resolve it to the immutable commit actually loaded, load the tokenizer from that same commit, leave the model's position configuration unchanged, and reject `--max_length` above a declared `max_position_embeddings` limit.

For a local checkpoint whose `tokenizer/` uses custom Python through an `auto_map`, pass `--trust_remote_code` only after inspecting that saved code. The flag is false by default and affects local checkpoint loading; a local directory does not remove the code-execution risk. Hub loading retains its existing policy.

The default dataset is `wikipedia` (`20220301.en`, `train`). Override it with `--dataset_name`, `--dataset_config`, and `--dataset_split`, or pass `--data_path` for a dataset saved with `Dataset.save_to_disk()`. The utility applies one inclusive `--min_chars`/`--max_chars` filter, then deterministic shuffling and optional `--num_dataset_shards`/`--dataset_shard_index` sharding. Rows that tokenize to no non-special positions are skipped with a warning instead of aborting the run.

Results default to `results/pseudo_perplexity/<model>/<checkpoint>/...csv`; Hub checkpoints use their resolved commit as the checkpoint directory, and `--output_path` changes the root. Each CSV has a manifest recording the concrete model/config/tokenizer source, custom-tokenizer trust policy, prepared dataset fingerprint and selection/sharding contract, length filters, seed, batch size, context length, and precision. Reruns skip completed sample IDs only when that manifest matches exactly; mutable Hub refs and replaced datasets therefore cannot silently mix results across an interrupted run. A mismatched manifest stops the run; choose a different `--output_path` when intentionally starting a new contract. On resume, every CSV field is validated; an incomplete or inconsistent tail is atomically removed from its first bad record onward so those samples are recomputed. Evaluation uses bf16 autocast by default; pass `--no-bf16` for fp32, and use `--device` and `--compile` to control placement and compilation.
