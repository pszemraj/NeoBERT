...
| `optimizer.name=muonclip` with FSDP sharding enabled                              | **ERROR**          | MuonClip requires full 2D matrices; use AdamW for sharded FSDP. MuonClip+FSDP is only allowed with `num_processes=1` or `NO_SHARD`. |
