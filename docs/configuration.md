...
- **All training entrypoints use `trainer.mixed_precision`** for mixed precision.
- **GLUE requires pretrained weights** unless `glue.allow_random_weights: true` is set. Provide `glue.pretrained_checkpoint_dir`, `glue.pretrained_checkpoint`, and `glue.pretrained_model_path`.
- **GLUE from Hub**: set `model.from_hub: true` and `model.name: <hf_id>` to load a hosted model instead of local checkpoints.
- **MTEB pooling**: set `mteb_pooling` to `"avg"` or `"cls"`; any other value falls back to CLS pooling (the default `"mean"` currently behaves like `"cls"`).
- **Contrastive training** requires `dataset.path` to point to the preprocessed dataset generated by `scripts/contrastive/preprocess.py`.
...
- **Vocab rounding**: the pretraining script always rounds vocab size to a multiple of 128 at startup for GPU efficiency. `trainer.use_cpu` is legacy and does not currently disable this path.
...
mteb_pooling: str = "mean"  # NeoBERTForMTEB expects "avg" or "cls" (default "mean" falls back to CLS)
...
```
model.name: str | None = None  # HuggingFace model id/path (GLUE/Hub workflows)
...
Note: when `dataset.pre_tokenize: true`, the pretokenized dataset is written to `dataset.pre_tokenize_output` if provided, otherwise to `tokenized_data/<dataset-name>/`.
...
Note: when `tokenizer.path` is set, training scripts prefer it over `tokenizer.name`.
...
trainer.dataloader_num_workers: int = 0  # legacy; pretraining uses dataset.num_workers instead
trainer.use_cpu: bool = False  # legacy; does not disable vocab rounding in pretraining

trainer.gradient_checkpointing: bool = False
trainer.gradient_clipping: float | None = None
trainer.mixed_precision: Literal["no", "bf16", "fp16"] = "bf16"
trainer.tf32: bool = True
trainer.torch_compile: bool = False  # opt-in; skipped if torch.compile unavailable or DeepSpeed active
trainer.log_weight_norms: bool = False

trainer.resume_from_checkpoint: bool | str | None = None  # true/"latest"/"auto" resumes newest in output_dir/checkpoints; string loads that path
...
wandb.log_interval: int = 100  # deprecated for trainer logging; use trainer.logging_steps
...
glue.pretrained_model_path: str | None = None  # path to pretraining config.yaml
...
Note: legacy GLUE keys under `model:` are mapped into `glue.*` with warnings. Prefer the `glue.*` section.
...
If you still have legacy GLUE keys under `model:` (e.g., `model.pretrained_config_path`),
they are normalized into `glue.*` during config loading with explicit warnings.
