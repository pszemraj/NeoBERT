...
def load_pretrained_model(checkpoint_dir, checkpoint_step, config_path, tokenizer_name):
    ...
    # Load checkpoint
    checkpoint_path = os.path.join(checkpoint_dir, "model_checkpoints", str(checkpoint_step), "state_dict.pt")
...
def test_semantic_similarity(model, tokenizer, device="cuda"):
    ...
    for sent1, sent2 in test_pairs:
        # Tokenize
        tokens1 = tokenizer(sent1, return_tensors="pt", padding="max_length", max_length=32, truncation=True)
        tokens2 = tokenizer(sent2, return_tensors="pt", padding="max_length", max_length=32, truncation=True)
        
        input_ids1 = tokens1["input_ids"].to(device)
        input_ids2 = tokens2["input_ids"].to(device)
        
        # Create masks
        mask1 = torch.where(tokens1["attention_mask"] == 1, 0.0, -float("inf")).to(device)
        mask2 = torch.where(tokens2["attention_mask"] == 1, 0.0, -float("inf")).to(device)
...
def test_sentiment_discrimination(model, tokenizer, device="cuda"):
    ...
    for sentence, true_label in test_sentences:
        # Tokenize
        tokens = tokenizer(sentence, return_tensors="pt", padding="max_length", max_length=32, truncation=True)
        ...
        print(f"\nSentence: {sentence}")
        print(f"  True Label: {true_label}, Predicted: {pred} {'✓' if is_correct else '✗'}")
...
def main():
    ...
    parser.add_argument("--config_path", type=str, required=True, help="Path to pretraining config")
    parser.add_argument("--tokenizer_name", type=str, default="BEE-spoke-data/wordpiece-tokenizer-32k-en_code-msp", 
    ...
    if accuracy > 0.6:
        ...
        print("⚠️  WARNING: Model performance is near random. More pretraining may be needed.")
