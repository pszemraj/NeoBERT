...
| `trainer.resume_from_checkpoint` with checkpoint `config.yaml` drift              | **CHECKPOINT WINS** | Resume forces checkpoint model shape/semantics, tokenizer identity, masking, and objective fields before constructing runtime objects; missing checkpoint config fails fast. Corpus identity (`dataset.name`/`config`/`path`/`cache_dir`/`text_column`) and, for RoPE, context length (`max_position_embeddings`/`tokenizer.max_length`/`max_seq_length`) stay launch-controlled for continued-pretraining, and drift is warned not reverted. |
