#!/usr/bin/env bash
# Launch NeoBERT pretraining with MuonClip + FP8 + FSDP2 (NO_SHARD) on 4 GPUs.
...
TRAIN_CONFIG="${TRAIN_CONFIG:-configs/pretraining/pretrain_neobert_fp8_muon_no_shard_4gpu.yaml}"
ACCELERATE_CONFIG="${ACCELERATE_CONFIG:-configs/accelerate/fsdp2_no_shard_fp8_4gpu.yaml}"
RUN_TAG="${RUN_TAG:-muon-fp8-fsdp2-no-shard-4gpu-$(date +%Y%m%d-%H%M%S)}"
...
echo "Launching MuonClip + FP8 + FSDP2 NO_SHARD pretraining on CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES}"
