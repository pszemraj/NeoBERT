...
- Output defaults to the MTEB result cache rooted at `outputs/<run>/mteb/<ckpt>/<max_length>/`; task files live at `results/NeoBERT__<model>/<contract-hash>/<task>.json`. The contract hash covers the resolved checkpoint, canonical pooling strategy, and requested context length, so a shared `--output_folder` remains safe across checkpoints and scoring settings. `mteb_overwrite_results: true` forces recomputation for an otherwise identical contract.
