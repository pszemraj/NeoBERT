...
- `src/neobert/huggingface/modeling_neobert.py` already supports packed varlen:
  - accepts `cu_seqlens` and `max_seqlen`
  - uses `flash_attn.flash_attn_varlen_func`
  - packed inputs are shape `(1, total_tokens)` and RoPE uses `position_ids`.
- `src/neobert/huggingface/modeling_neobert.py::DataCollatorWithPacking` already
  constructs `cu_seqlens`, `max_seqlen`, and `position_ids`.
...
- Model:
  - either add a varlen path to `src/neobert/model/model.py` using flash-attn
    (matching HF logic), or introduce a training wrapper that calls the HF model
    for varlen batches only.
  - require `flash_attn` on GPU; error clearly if missing.
  - keep additive-mask path unchanged for non-packed batches.

Constraints / gotchas:
- RoPE in packed mode needs correct `position_ids` per segment. The HF collator
  already provides these; the training path must consume them.
- `flash_attn_varlen_func` expects q/k/v shaped `[total_tokens, nheads, head_dim]`
  with `cu_seqlens` and `max_seqlen` (see HF implementation for reference).
...
- `src/neobert/huggingface/modeling_neobert.py` (EncoderBlock varlen path and
